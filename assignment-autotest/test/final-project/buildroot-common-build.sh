...
# Validating if raspi-theramin.mk has ssh link
grep "^RASPI_THERAMIN_SITE[[:space:]]*=[[:space:]]*https" base_external/package/raspi-theramin/raspi-theramin.mk
rc=$?
if [ $rc -eq 0 ]; then
	add_validate_error "Using https instead of ssh URL in raspi-theramin.mk, will not build automated"
	echo "Replacing https link by ssh link manually in raspi-theramin.mk"
	sed -i 's/^RASPI_THERAMIN_SITE[[:space:]]*=[[:space:]]*https:\/\/github.com\//RASPI_THERAMIN_SITE = git@github.com:/g' base_external/package/raspi-theramin/raspi-theramin.mk
fi
